...
目前是 **Phase 3A：本機提醒偏好設定**。可以完成首次設定、快速記水、修改紀錄、查看七日趨勢，以及管理個人目標、常用容器與未來的提醒時段。資料保存在瀏覽器的 IndexedDB；App 在線開啟一次後，也能安裝到裝置並在離線時重新啟動。
...
- 開關本機提醒偏好，並設定每天的開始、結束時間與 30／60／90 分鐘間隔。

提醒設定目前只保存在本機，尚不會要求通知權限或發送系統通知。
...
- 不發送通知、背景提醒或 Web Push；本階段只保存未來要使用的提醒偏好。
...
├── SettingsView.tsx           個人目標、提醒、容器、示範與資料清除
└── types.ts                   共用資料型別
```
